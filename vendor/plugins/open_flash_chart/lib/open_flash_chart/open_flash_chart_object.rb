...
module OpenFlashChart
  module Controller
    def open_flash_chart_object(width, height, url, class_name="chart-class", swf_file_name="open-flash-chart.swf", use_swfobject=true, base="/")
      get_object_values(url)
      get_html(@ofc_url, @div_name, base, swf_file_name, width, height, @protocol, @obj_id, class_name)
    end

    # if you want the div name back for working with js, this is the ticket
    def open_flash_chart_object_and_div_name(width, height, url, class_name="chart-class", swf_file_name="open-flash-chart.swf", use_swfobject=true, base="/")
      get_object_values(url)
      html = get_html(@ofc_url, @div_name, base, swf_file_name, width, height, @protocol, @obj_id, class_name)
    ...
    def get_html(url, div_name, base, swf_file_name, width, height, protocol, obj_id, class_name)
