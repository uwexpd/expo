class Apply::ProceedingsController < ApplyController
  before_filter :check_if_uses_proceedings
  ...
  def index
    ...
    respond_to do |format|
      format.html
      format.iphone
      format.pdf if @current_user.admin?
    end
  end
  ...
  def result
    ...
    respond_to do |format|
      format.html
      format.iphone
      format.pdf { render :layout => 'proceedings', :merge_with => merge_file } if @current_user.admin? || @result.size <= 25
    end
  end
